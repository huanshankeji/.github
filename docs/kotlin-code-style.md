...
1. Preserve existing comments and blank lines when editing a file. Do not remove or collapse them on your own — only when the user asks, or when the code they belong to is removed or rewritten as part of the requested change. See also [general-agent-instructions.md — Editing existing files](general-agent-instructions.md#editing-existing-files).
